...
<div class="page-container">
	<table class="table table-border table-bordered table-bg">
		...
		<tbody>
			...
			<c:forEach items="${myborrow}" var="book" varStatus="vs">
			<tr class="text-c">
				<td>${book.bookid }</td>
				<td>${book.isbn }</td>
				<td>${book.bookname }</td>
				...
				<td><c:if test="${book.part eq '0'}">预借阅</c:if>
				...
				<td>${book.time }天</td>
				<td >
				<c:if test="${book.part eq '0'}"><a href="${pageContext.request.contextPath}/user/delborrow?id=${book.id}">取消预借阅</a></c:if>
				<c:if test="${book.part eq '1'}">借阅中,暂无操作</c:if>
